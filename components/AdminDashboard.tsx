...
// Worker should be set globally by the App or UserViewer to avoid conflicts.
// If not set, we can rely on UserViewer's setup or set it safely here without CDN if possible.
// For now, removing the CDN override which is likely the cause of the breakage.
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onLogout }) => {
  ...
  // .. (Rest of state remains same, not re-writing all for brevity if not changing)

  // Need to provide the Rest of the component code until handleFileUpload to ensure context match
  // Since replace_file_content replaces the block, I need to match start/end carefully.
  // The user wants "Hemen eski haline getir". 

  // I will target specific blocks to minimize disruption.
  ...
  const loadData = async () => {
    ...
    try {
      if (adminTab === 'KEYS') {
        const [cl, bk, ks] = await Promise.all([
        ...
        if (activeFolderId) {
          const contents = await StorageService.getFolderContent(activeFolderId);
          setFolderContents(contents);
        }
        // Fetch all files for selection in key creation
        setAllFiles(await StorageService.getAllFiles());
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (file.name.endsWith('.pdf')) {
        // Removed dynamic import and workerSrc override
